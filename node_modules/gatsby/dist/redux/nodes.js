...
function postIndexingMetaSetupLtLteGtGte(filterCache, op) {
  ...
  entries.forEach(([v, bucket]) => {
    bucket.sort((a, b) => a.internal.counter - b.internal.counter); // Record the range containing all nodes with as filter value v
...
const getNodesFromCacheByValue = (filterCacheKey, filterValue, filtersCache) => {
  ...
  if (op === `$eq`) {
    ...
    if (filterValue == null) {
      ...
      return ((_filterCache$byValue$ = filterCache.byValue.get(null)) !== null && _filterCache$byValue$ !== void 0 ? _filterCache$byValue$ : []).concat((_filterCache$byValue$2 = filterCache.byValue.get(undefined)) !== null && _filterCache$byValue$2 !== void 0 ? _filterCache$byValue$2 : []);
    }
  ...
  if (op === `$in`) {
    ...
    const arr = []; // Note: it's very unlikely that the list of filter values is big so .includes should be fine here
  ...
  if (op === `$lt`) {
    ...
    if (range) {
      return nodes.slice(0, range[0]).sort((A, B) => A.internal.counter - B.internal.counter);
    ...
    return nodes.slice(0, until).sort((A, B) => A.internal.counter - B.internal.counter);
  }
  ...
  if (op === `$lte`) {
    ...
    if (range) {
      return nodes.slice(0, range[1]).sort((A, B) => A.internal.counter - B.internal.counter);
    } // Query may ask for a value that doesn't appear in the list, like if the
    // list is [1, 2, 5, 6] and the query is <= 3. In that case we have to
    // apply a search (we'll do binary) to determine the offset to slice from.
    // Note: for lte, the valueAsc array must be list at this point
  ...
  if (op === `$gt`) {
    ...
    if (range) {
      return nodes.slice(0, range[0]).reverse().sort((A, B) => A.internal.counter - B.internal.counter);
    } // Query may ask for a value that doesn't appear in the list, like if the
    // list is [1, 2, 5, 6] and the query is <= 3. In that case we have to
    // apply a search (we'll do binary) to determine the offset to slice from.
    ...
    const until = pivotValue > filterValue ? inclPivot : exclPivot;
    return nodes.slice(0, until).reverse().sort((A, B) => A.internal.counter - B.internal.counter);
  }
  ...
  if (op === `$gte`) {
    ...
    if (range) {
      return nodes.slice(0, range[1]).reverse().sort((A, B) => A.internal.counter - B.internal.counter);
    } // Query may ask for a value that doesn't appear in the list, like if the
    // list is [1, 2, 5, 6] and the query is <= 3. In that case we have to
    // apply a search (we'll do binary) to determine the offset to slice from.
    // Note: for gte, the valueDesc array must be list at this point
